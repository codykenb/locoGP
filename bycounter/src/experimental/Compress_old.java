...
final class Code_Table {
	...
	public Code_Table() {
		this.tab = new short[Compress_old.HSIZE];
	}
...
class Comp_Base {
	...
	public Comp_Base(Input_Buffer in, Output_Buffer out) {
		this.Input = in;
		this.Output = out;
		this.maxbits = Compress_old.BITS;
		this.block_compress = Compress_old.BLOCK_MASK;
		this.buf = new byte[Compress_old.BITS];
	}
...
final class Compressor extends Comp_Base {
	...
	public Compressor(Input_Buffer in, Output_Buffer out) {
		super(in, out);
		if (this.maxbits < Compress_old.INIT_BITS)
			this.maxbits = Compress_old.INIT_BITS;
		if (this.maxbits > Compress_old.BITS)
			this.maxbits = Compress_old.BITS;
		this.maxmaxcode = 1 << this.maxbits;
		this.n_bits = Compress_old.INIT_BITS;
		...
		this.free_ent = ((this.block_compress != 0) ? Compress_old.FIRST : 256);

		this.htab = new Hash_Table(); // dm/kmd 4/10/98
		this.codetab = new Code_Table();

		this.Output.putbyte(Compress_old.magic_header[0]);
		this.Output.putbyte(Compress_old.magic_header[1]);
	...
	private void cl_block() {
		...
		if (rat > this.ratio) {
			...
			this.ratio = 0;
			this.htab.clear();
			this.free_ent = Compress_old.FIRST;
			this.clear_flg = 1;
	...
	public void compress() {
		...
		next_byte: while ((c = this.Input.getbyte()) != -1) {
			this.in_count++;
	...
	private void output(int code) {
		...
		if (code >= 0) {
			 ...
			 * on the left.
			 */
			this.buf[bp] = (byte) ((this.buf[bp] & Compress_old.rmask[r_off]) | (code << r_off)
