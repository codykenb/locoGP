...
public class Benchmarker {
	...
	private static void testProbabilityPropagation() {
		...
		Iterator<ASTNode> iter = ind.gpMaterial.getAllNodes().iterator();
		while(iter.hasNext()){
			tempNode = iter.next();
	...
	private static void printIndividualProbabilities(Individual tempInd){
		GPASTNodeData gpData = null;
		ASTNode tempNode = null;
		Iterator<ASTNode> iter = tempInd.gpMaterial.getAllNodes().iterator();
		while(iter.hasNext()){
			tempNode = iter.next();
			gpData =(GPASTNodeData) tempNode.getProperty("gpdata"); 
			System.out.println( "\n"+gpData.getModifyProbability()+" : "+tempNode.toString());
		}
