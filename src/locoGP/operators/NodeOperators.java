...
public class NodeOperators {
	...
	private static float nodeSelectionTournamentSizeRatio = .2f;
	...
	public static Individual crossover(Individual parentOne,
		...
		try {
			do {				
				...
				nodeToReplace = pickNodes(newInd.gpMaterial.getAllNodes(),1,pickBestLocation).get(0);
	...
	public static boolean replaceNode(ASTNode nodeToReplace,
	...
	public static int mutate(Individual newInd, int choice, int numChanges,
		...
		List<ASTNode> mutationCandidates = newInd.gpMaterial.getAllNodes();
	...
	private static List<ASTNode> pickBestNodes(
		...
		ASTNode curNode;
		for (Integer curIndex : indexToPick) { // find best out of these
			...
			if (((GPASTNodeData) curNode.getProperty("gpdata"))
					.getModifyProbability() > ((GPASTNodeData) returnNode
					.getProperty("gpdata")).getModifyProbability()) {
				returnNode = curNode;
			}
		}
	...
	private static void deleteNode(ASTNode stmt) {
		try {
			stmt.delete();
	...
	public static CompilationUnit cloneAST(CompilationUnit originalAST) {
		...
		GPMaterialVisitor.cloneProbabilitiesDownTree(originalAST, cloneAST); 
		return cloneAST;
	}
