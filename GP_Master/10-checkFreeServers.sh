...
tempServers=$(parallel ssh -oBatchMode=yes -o ConnectTimeout=5 {} "hostname \; who \;ps faux \\| grep loco \| grep -v grep \; cat /proc/loadavg \| awk \'{ print \\\$1 }\' "  ::: ${servers[@]} 2> /dev/null  | grep -B1 "^0" | grep -v tty | grep -v \, | grep -v pts | grep -v ^0 | grep -v ^1 | grep -v \\-\\- | grep -v \:0 | grep -v \:1)
