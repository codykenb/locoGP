...
# this puts a limit on the depth of method calls in a program (maybe stackoverflow programs should not receive the worst fitness?)
	$java6fullpath -server -Xss500M -Xmx3072M -XX:ReservedCodeCacheSize=256M -XX:+UseCodeCacheFlushing -XX:MaxPermSize=2072M -XX:+ScavengeBeforeFullGC -XX:+UseConcMarkSweepGC -XX:+CMSClassUnloadingEnabled -XX:+CMSPermGenSweepingEnabled -jar ../*.jar >  /dev/null 2> errorCrap.txt & 
	javaPID=$!
  sleep 30
